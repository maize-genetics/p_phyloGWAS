...
- **Large T1 raw/reference data, deposited externally** — marked "external, link TBD" below
  ...
  agDataCommons; GIS raster data → Zenodo; DEG/reference-CDS/expression data → each item's own
  published-dataset source; `Zm00001eb.1.fulldata_goList.txt` → MaizeGDB (not planned to be
  hosted anywhere by this project at all, just cite the source); ESM2/PlantCAD → HuggingFace
...
| Maize reference CDS | `data/Zmays_cds.fa` (external published dataset — link TBD) | orthology, molecular evolution | T1 |
| Angiosperms353 Oryza reference sequences | `data/Angiosperms353_orysaSequences.fasta` (external published dataset — link TBD) | 03C (miniprot cross-mapping) | T1 |
| Maize B73 v5 mRNA reference | `data/Zea_mays_v5_mrna.fa` (external published dataset — link TBD) | 03C (miniprot cross-mapping) | T1 |
| Per-species CDS FASTAs (a-priori stress genes) | `data/DEG_CDS_fasta/` (external published dataset(s) — link TBD) | 10A | T1 |
...
| Maize v5 max-expression (FPKM) | `data/maxFPKM_v5.csv` (external published dataset — link TBD) | expression-informed filtering | T1 |
