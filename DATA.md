...
- **Large T1 raw/reference data, deposited externally** — marked "external, link TBD" below
  ...
  agDataCommons; GIS raster data → Zenodo; `data/Angiosperms353_orysaSequences.fasta` → its own
  published-dataset source; `data/Zmays_cds.fa`, `data/Zea_mays_v5_mrna.fa`,
  ...
  ESM2/PlantCAD → HuggingFace (already resolved, pulled at runtime, see "Model weights" below);
  GBIF/BIEN occurrence records → live API query, or the existing Zenodo `14968186` shortcut.
