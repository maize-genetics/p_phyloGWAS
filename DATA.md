...
| 32 representative genome assemblies + Helixer gene annotations | genome/annotation content: live rsync from `cbsublfs1:/data1/users/coh22/poaceae_tfbs/og_consensus` (not stored locally — pulled at runtime by `03A_buildHelixerOG.sh`, then translated to protein via `gffread`), so these aren't a separate hosting entry — they're a named subset of the row-1 `data/assemblies/` set above. Which 32: `data/poaceaeRepAssemblies_32.txt` (tracked on GitHub) — extracted from a real historical OrthoFinder run's `SpeciesIDs.txt` (`output/OrthoFinder/Results_Jun06/`). **31 of the 32 match an assembly ID currently in `data/assemblies/`; one, `Pp-Kellogg1297-DRAFT-PanAnd-1.0`, does not** (no exact or near-name match found) — likely renamed/replaced/dropped in `data/assemblies/` since that June 2024 run, since the rsync source is externally maintained and not version-locked to this repo's own assembly set. Flagged, not resolved. | 03 (OrthoFinder input) | T1 |
