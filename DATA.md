...
- **Large T1 raw/reference data, deposited externally** — marked "external, link TBD" below
  ...
  agDataCommons; GIS raster data → Zenodo; `data/DEG_CDS_fasta/` and
  `data/Angiosperms353_orysaSequences.fasta` → each item's own published-dataset source;
  `data/Zmays_cds.fa`, `data/Zea_mays_v5_mrna.fa`, `data/maxFPKM_v5.csv`, and
  `data/Zm00001eb.1.fulldata_goList.txt` → MaizeGDB (not planned to be hosted anywhere by this
  project at all, just cite the source — resolved, no link needed); ESM2/PlantCAD → HuggingFace
