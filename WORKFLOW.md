...
| **04** `msaGeneration` | Per-OG CDS multiple sequence alignment (mafft) | Filtered OG list + CDS sequences (from 03) | `notebook/04_msaGeneration/README.md` — `mafft --ep 0 --genafpair --maxiterate 1000 <input> > <output>` | Per-OG MSAs (`output/OrthofinderMAFFT/*_mafft.fa`, T2/regenerable; on this machine a renamed copy currently sits at `output/CDSMSAPerOG/` — see the stage-04 README's note) → 05 (gap-stripping/gene trees), 07 (dN/dS calculation needs the MSA directly), 09 (HyPhy RELAX needs the MSA directly) |
...
- **dN/dS to reference** (new `07Bb_dNdS_run.sh`): `src/07B_getOmega2Ref.R` was already
  ...
  gap-stripped MSAs, rather than the raw pre-gap-stripping `output/OrthofinderMAFFT/*_mafft.fa`
  this step originally read — `07B_getOmega2Ref.R`'s reference-based gap-stripping is a no-op
  on already-stripped input, so this is a harmless simplification) with
...
| [Helixer](https://github.com/weberlab-hhu/Helixer) | — (external repo) | 03 (gene annotation for the 32 representative assemblies, see `DATA.md`) | not applicable — run outside this repo, not deposited/hosted |
