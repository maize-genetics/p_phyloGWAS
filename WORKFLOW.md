...
The pipeline runs as a numbered sequence of stages (`02` → `10`; there is no `00`/`01`/`04`
stage). Each row below maps in the data it needs (cross-referenced to `DATA.md`), the
scripts/notebooks that do the work, and what it hands off to the next stage.
...
| **03** `orthogroup` | OrthoFinder (32 representative genomes) → orthogroups → ancestral AA sequence reconstruction → miniprot cross-mapping → OG filtering | Genome assemblies; dN/dS table; rice→OG mapping; OG→maize mapping; miniprot GFF annotations | `03A_buildHelixerOG.sh`, `03B_OGFilter.ipynb`, `03C_miniProtResult_eval.ipynb`, `03D_OGtranslation.R` | Filtered OG list + ancestral sequences → 05, 07 |
