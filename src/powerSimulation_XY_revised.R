...
shift_Y       <- 2
beta_range    <- seq(0, 0.5, by = 0.05)
n_X           <- 1   # X traits per replicate
n_sims        <- 500    # simulation replicates per (beta, scenario, prop)
alpha         <- 0.05
n_cores       <- 10
MASTER_SEED   <- 42
...
fit_model <- function(data, Kmat, responseVar, predictor) {
  asreml.options(verbose = FALSE, trace = FALSE,maxit = 60)
...
# =============================================================================
# 8. MAIN LOOP  (replaces 16 individual mclapply calls)
#
#    Design grid: 4 scenarios × 4 prop levels × 21 beta values × n_sims reps
#    Each mclapply parallelises over beta_range for a fixed (scenario, prop).
#    RNGkind("L'Ecuyer-CMRG") + set.seed() before each mclapply ensures that
#    results are fully reproducible regardless of n_cores.
# =============================================================================

prop_levels <- c(1.0, 0.8, 0.5, 0.3)
prop_labels <- c("100%", "80%", "50%", "30%")

RNGkind("L'Ecuyer-CMRG")   # must be set once before any parallel RNG use
...
for (sc_name in names(scenarios)) {
  ...
  for (pi in seq_along(prop_levels)) {
    prop  <- prop_levels[pi]
    plabel <- prop_labels[pi]

    cat(sprintf("\n[%s | prop = %s]\n", sc_name, plabel))

    # Derive a deterministic seed for this (scenario, prop) combination so
    # results are reproducible across runs without fixing a global state
    combo_seed <- MASTER_SEED +
                  match(sc_name, names(scenarios)) * 100 + pi * 10

    set.seed(combo_seed)   # seeds the L'Ecuyer streams for child processes

    curve_results <- mclapply(seq_along(beta_range), function(bi) {
      ...
      for (s in seq_len(n_sims)) {
        # Each replicate gets a unique seed derived from combo + beta + rep
        sim_seed <- combo_seed + bi * 1000 + s

        # Draw nodes: fixed set reused, random set redrawn every replicate
        if (sc$type == "fixed") {
          rep_nodes <- sc$nodes
        } else {
          rep_nodes <- sample_independent_nodes(tree, sc$n,
                                                seed = sim_seed)
        }

        res <- tryCatch(
          run_single_replicate(phyloKMat, tree, rep_nodes, prop,
                               sigma2_Y, sigma2_X, shift_Y, b, n_X,
                               sigma2_resid, alpha, sim_seed),
          error = function(e) {
            warning(sprintf("Replicate failed (beta=%.3f, sim=%d): %s",
                            b, s, conditionMessage(e)))
            list(hits = 0L, total = 0L)
          }
        )
        total_hits  <- total_hits  + res$hits
        total_valid <- total_valid + res$total
      }

      power  <- total_hits / total_valid
      ci     <- wilson_ci(total_hits, total_valid)
      return(c(beta = b, power = power, ci_lower = ci[1], ci_upper = ci[2],
        hits = total_hits, total = total_valid))

    }, mc.cores = n_cores)

    df <- as.data.frame(do.call(rbind, curve_results))
    df$scenario    <- sc_name
    df$convergence <- plabel

    all_results[[paste(sc_name, plabel)]] <- df
    cat(sprintf("  done (%d beta values x %d sims)\n",
                length(beta_range), n_sims))
  }
}

power_df_merged <- do.call(rbind, all_results)
...
power_df_merged$convergence <- factor(power_df_merged$convergence,
                                       levels = prop_labels)
colnames(power_df_merged) = c("beta","power","ci_lower","ci_upper","hits","total","scenario","convergence")
# =============================================================================
# 9. WRITE FULL RESULTS
# =============================================================================

write.csv(power_df_merged,
          file.path(OUTPUT_DIR, "power_df_merged.csv"),
          row.names = FALSE)

cat("\nFull results written to power_df_merged.csv\n")
